...
async function apiUpdateSupplier(id, payload){
    ...
    const idx = state.suppliers.findIndex(s=>String(s.id)===String(id));
...
function wireEvents(){
    ...
    $("materialForm").addEventListener("submit", async (e)=>{
        ...
        const payload = {
            ...
            unit: $("matUnit").value,
            stockQty: Number($("matStock").value),
        ...
        if(payload.stockQty < 0 || payload.lowThreshold < 0){
    ...
    $("stockForm").addEventListener("submit", async (e)=>{
        ...
        try{
            ...
            if(action === "remove" && (Number(m.stockQty||0) - qty) < 0){
