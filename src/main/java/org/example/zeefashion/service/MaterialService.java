...
public class MaterialService {
    ...
    public Material update(Long id, Material payload) {
        ...
        if (payload.getLowThreshold() != null) m.setLowThreshold(payload.getLowThreshold());
        
        // Sync 'Supplier Stock' if provided
        if (payload.getSupplierStock() != null) {
            m.setSupplierStock(payload.getSupplierStock());
        } else if (payload.getStockQty() != null && (m.getSupplierStock() == null || m.getSupplierStock() == 0)) {
            // Backward compatibility / Seed sync
            m.setSupplierStock(payload.getStockQty());
        }
        
        if (payload.isExposedInInventory() != null) {
